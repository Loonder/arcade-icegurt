...
            vitorias INTEGER DEFAULT 0, /* Corrigido de 'vitarias' para 'vitorias' */
            FOREIGN KEY (usuario_id) REFERENCES usuarios(id)
        )
    `);
    
    // MIGRACAO CRITICA: Renomear a coluna (se existir) para corrigir o erro 'no such column: l.vitarias'
    // O erro nos logs sugere que o código usa vitarias, mas o banco usa vitorias, ou vice-versa.
    // O mais seguro é padronizar para 'vitorias' no código e tentar corrigir o nome da coluna existente.
    db.run(`
        ALTER TABLE leaderboard RENAME COLUMN vitarias TO vitorias;
    `, (err) => {
        if (err && !err.message.includes('no such column')) {
            // Este erro é esperado se a coluna já tiver o nome correto ('vitorias') ou for 'no such column'
            // Ignoramos a falha se a coluna não existe (DB novo) ou já está certa.
        }
    });
});
