...
class GameCoordinator {
  ...
  gameOver() {
    ...
    // 🚀 Envia a pontuação final para o sistema principal (Arcade)
    window.dispatchEvent(
      new CustomEvent("pacmanGameOver", { detail: { score: this.points } })
);
...
} // 👈 FECHA a classe Timer AQUI!


// 🔗 Comunicação com o Arcade principal
if (typeof window !== "undefined") {
  window.addEventListener("pacmanGameOver", (event) => {
    const score = event.detail?.score || 0;
    console.log("🏁 Pac-Man terminou com pontuação:", score);

    // Envia para o app.js do Arcade
    window.parent.postMessage(
      { tipo: "PACMAN_SCORE", pontos: score },
      "*"
    );
  });
}