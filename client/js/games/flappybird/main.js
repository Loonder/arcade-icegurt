...
// ### NOSSA INTEGRAÇÃO (INÍCIO) ###
// Variável para garantir que a pontuação seja enviada apenas uma vez
let scoreSent = false;
// ### NOSSA INTEGRAÇÃO (FIM) ###
...
            // ### NOSSA INTEGRAÇÃO (INÍCIO) ###
            // Se o jogo acabou E a pontuação ainda não foi enviada
            if (!scoreSent) {
              console.log("Enviando pontuação final para o Arcade:", score.current);
                // Envia a mensagem para o app.js (a janela "pai")
                window.parent.postMessage({ 
                    tipo: "FLAPPYBIRD_SCORE", 
                    pontos: score.current 
                }, "*");
                
                // Marca como enviado para não enviar de novo (evita spam)
                scoreSent = true; 
            }
            // ### NOSSA INTEGRAÇÃO (FIM) ###
       }
...
***** FUNCTIONS: ********
************************/
// Função para entrar em tela cheia (com prefixos de navegadores)
...
// ##### CÓDIGO NOVO (COLE ISTO NO LUGAR DO SEU 'LOOP') #####

// Novas variáveis para controlar o "Game Loop"
let lastTime = 0;
const targetFPS = 60; // Vamos travar o jogo em 60 FPS, como era antes
const frameInterval = 1000 / targetFPS; // ~16.7ms por frame

let loop = (timestamp) => {
    // 'timestamp' é fornecido automaticamente pelo requestAnimationFrame
    
 ...
    // Só roda a lógica do jogo se o tempo (frameInterval) tiver passado
    if (deltaTime >= frameInterval) {
        
        // Ajusta o 'lastTime' para evitar "drift" (acúmulo de atraso)
        lastTime = timestamp - (deltaTime % frameInterval);
        
        // Agora, estas funções rodam na velocidade correta (60x por seg)
        draw();
        update();
        frame++;
    }
    
    // Chama o próximo frame, não importa o que aconteça
    requestAnimationFrame(loop);
}

// Inicia o game loop otimizado
requestAnimationFrame(loop);
...
************************/
/**
 * Função centralizada para lidar com todas as ações do jogador
 * (Pular, Começar, Reiniciar)
 */
function handleAction() {
     if (gameState.current == gameState.getReady) {
         gameState.current = gameState.play;
         SFX_SWOOSH.play();
         // Tenta entrar em tela cheia no primeiro toque/clique
         toggleFullscreen(cvs); 
     } 
 ...
         SFX_SWOOSH.play();
         // Tenta entrar em tela cheia ao recomeçar
         toggleFullscreen(cvs);
     }
}

// 1. Ouvinte para o Mouse (Desktop)
cvs.addEventListener('click', () => {
     handleAction();
});

// 2. Ouvinte para o Toque (Mobile - SEM DELAY)
cvs.addEventListener('touchstart', (e) => {
     // Impede que o navegador tente dar scroll ou zoom junto
     e.preventDefault(); 
     handleAction();
}, { passive: false }); // Necessário para o e.preventDefault() funcionar

// 3. Ouvinte para o Teclado (Desktop - Barra de Espaço)
