...
Asteroids.playfield = function(game, home) {
    // tenta usar o canvas do seu sistema (id="game")
    var canvas = document.getElementById("game");

    // se não existir, cria um como fallback
    if (!canvas) {
        canvas = document.createElement('canvas');
        home.appendChild(canvas);
    } else {
        // se já existir em outro lugar, movemos para o wrapper correto (home)
        if (canvas.parentNode !== home) {
            try {
                home.appendChild(canvas);
            } catch (e) {
                // fallback: se não der pra mover, criamos um novo
                canvas = document.createElement('canvas');
                home.appendChild(canvas);
            }
        }
...
Asteroids.player = function(game) {
    ...
    // Helper: envia update live para o parent (se existir)
    function postLiveScore(s) {
        try {
            if (window && window.parent && window.parent !== window) {
                window.parent.postMessage({
                    tipo: "ASTEROIDS_SCORE_LIVE",
                    pontos: s
                }, "*");
            }
        } catch (e) {
            // silencioso: não queremos quebrar o jogo por causa disso
            game.log.debug("postLiveScore falhou: " + e);
        }
    }
    ...
    return {
        ...
        addScore: function(pts) {
            score += pts;
            // Envia atualização em tempo real para o app (se houver)
            postLiveScore(score);
        },
        ...
        extraLife: function(game) {
            ...
            lives++;
            // podemos avisar o parent se quiser (opcional)
            try {
                if (window && window.parent && window.parent !== window) {
                    window.parent.postMessage({
                        tipo: "ASTEROIDS_EXTRA_LIFE",
                        vidas: lives
                    }, "*");
                }
            } catch (e) {
                game.log.debug("post extra life falhou: " + e);
            }
        },
        die: function(game) {
            if (!dead) {
...
Asteroids.gameOver = function (game) {
    ...
    let scoreSent = false; // evita duplicação

    return function () {

        // Aqui está o score REAL do Asteroids
        const pontuacaoFinal = game.player.getScore();

        // High Score original
        if (pontuacaoFinal > 0) {
            game.highScores.addScore('Player', pontuacaoFinal);
        }

        // --- ENVIO PARA O ARCADE ---
        if (!scoreSent) {
            try {
                console.log("Enviando pontuação final do Asteroids para o Arcade:", pontuacaoFinal);

                window.parent.postMessage({
                    tipo: "ASTEROIDS_SCORE",
                ...
                console.warn("Falha ao enviar ASTEROIDS_SCORE:", e);
            }
        }
        // --- FIM ---
...
// Public init helper: call this from your launcher:
// initAsteroids(homeElement) -> returns the Asteroids instance.
window.initAsteroids = function(homeEl) {
    var home = homeEl || window.ASTEROIDS_HOME || document.getElementById('gameContainer') || document.getElementById('game') || document.getElementById('asteroids');
    if (!home) {
        console.warn('initAsteroids: no container found (pass a DOM element as argument).');
        return null;
    }
    return new Asteroids(home);
};

// Auto-initialize if we can find a sensible container on load.
// This keeps backwards-compatibility with pages that include <div id="asteroids">.
(function() {
    function tryAutoInit() {
        var autoHome = window.ASTEROIDS_HOME || document.getElementById('gameContainer') || document.getElementById('game') || document.getElementById('asteroids');
        if (autoHome) {
            // Only auto-init if the container is present and non-empty.
            // This prevents the game from auto-starting in apps that don't want it.
            initAsteroids(autoHome);
        }
    }
